...
var require_server = __commonJS({
  "node_modules/qrcode/lib/server.js"(exports2) {
    ...
    exports2.toFile = function toFile(path3, text, opts, cb) {
...
function folderColor(path3) {
  const folder = path3.includes("/") ? path3.slice(0, path3.indexOf("/")) : "root";
  const hue = hashText(folder) % 360;
  return `hsl(${hue}, 72%, 64%)`;
}
function initialPosition(path3) {
  const hash = hashText(path3);
  const azimuth = (hash & 65535) / 65535 * Math.PI * 2;
  const vertical = (hash >>> 16 & 65535) / 65535 * 2 - 1;
...
function isExcluded(file, folders, tags, app) {
  ...
  const fileTags = new Set(
    (cache ? (0, import_obsidian.getAllTags)(cache) ?? [] : []).map((tag) => tag.toLocaleLowerCase())
  );
  return tags.some((tag) => fileTags.has(tag.toLocaleLowerCase()));
}
function exportVaultGraph(app, excludedFolders, excludedTags) {
  const files = app.vault.getMarkdownFiles().filter((file) => !isExcluded(file, excludedFolders, excludedTags, app));
...
var PairingModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    void import_qrcode.default.toCanvas(canvas, this.pairingUrl, {
      ...
      console.error("Obsidian AR n\xE3o conseguiu desenhar o QR Code.", error);
...
var ObsidianArPlugin = class extends import_obsidian2.Plugin {
  ...
  async exportGraph(showNotice) {
    ...
    if (!root) {
      if (showNotice) new import_obsidian2.Notice("Configure a pasta do projeto Obsidian AR.");
      return;
    }
...
`, "utf8");
  ...
  async startAr(report) {
    ...
    const root = this.settings.projectRoot.trim();
    if (!root) {
      new import_obsidian2.Notice("Abra Configura\xE7\xF5es \u2192 Obsidian AR e informe a pasta do projeto.");
    ...
    this.startPromise = (async () => {
      try {
        ...
        new import_obsidian2.Notice("Obsidian AR: preparando grafo, ponte e t\xFAnel\u2026", 8e3);
        ...
        new import_obsidian2.Notice("Obsidian AR pronto para parear com o Quest.");
        return true;
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        console.error("Obsidian AR n\xE3o iniciou.", error);
        this.setSessionStatus(`Falha: ${message}`, report);
        new import_obsidian2.Notice(`Obsidian AR: ${message}`, 12e3);
        return false;
      } finally {
  ...
  async stopAr() {
    ...
    try {
      ...
      new import_obsidian2.Notice("Sess\xE3o Obsidian AR encerrada.");
    } catch (error) {
...
var ObsidianArSettingTab = class extends import_obsidian2.PluginSettingTab {
