...
async function bitmap(url, optional = false) {
  const response = await resilientFetch(url, { mode: "cors", credentials: "omit", cache: "force-cache" });
...
async function resolveTaxonKey(taxon) {
  const response = await resilientFetch(`https://api.gbif.org/v1/species/match?name=${encodeURIComponent(taxon)}`, {
...
async function occurrenceClusters(config, taxonKey, center) {
  ...
  const response=await resilientFetch("https://api.gbif.org/v1/occurrence/search?"+query,{credentials:"omit",cache:"force-cache"});
...
const BRAZIL_STATE_NAMES={11:"Rondônia",12:"Acre",13:"Amazonas",14:"Roraima",15:"Pará",16:"Amapá",17:"Tocantins",21:"Maranhão",22:"Piauí",23:"Ceará",24:"Rio Grande do Norte",25:"Paraíba",26:"Pernambuco",27:"Alagoas",28:"Sergipe",29:"Bahia",31:"Minas Gerais",32:"Espírito Santo",33:"Rio de Janeiro",35:"São Paulo",41:"Paraná",42:"Santa Catarina",43:"Rio Grande do Sul",50:"Mato Grosso do Sul",51:"Mato Grosso",52:"Goiás",53:"Distrito Federal"};
let brazilStatesPromise;const stateOverlayCache=new Map();
async function brazilStates(){if(!brazilStatesPromise)brazilStatesPromise=fetch("/vendor/species-map/brazil-states.geojson?v=1",{cache:"force-cache"}).then(r=>{if(!r.ok)throw Error("Brazil states HTTP "+r.status);return r.json()});return brazilStatesPromise}
function statePolygons(geometry){if(!geometry)return[];return geometry.type==="MultiPolygon"?geometry.coordinates:geometry.type==="Polygon"?[geometry.coordinates]:[]}
function pointInRing(point,ring){let inside=false;for(let i=0,j=ring.length-1;i<ring.length;j=i++){const a=ring[i],b=ring[j];if(((a[1]>point[1])!==(b[1]>point[1]))&&(point[0]<(b[0]-a[0])*(point[1]-a[1])/((b[1]-a[1])||1e-9)+a[0]))inside=!inside}return inside}
function pointInState(point,geometry){return statePolygons(geometry).some(polygon=>pointInRing(point,polygon[0])&&!polygon.slice(1).some(hole=>pointInRing(point,hole)))}
function drawStatePath(context,geometry){context.beginPath();for(const polygon of statePolygons(geometry))for(const ring of polygon){ring.forEach(([lon,lat],index)=>{const q=brazilPoint(lon,lat);index?context.lineTo(q.x,q.y):context.moveTo(q.x,q.y)});context.closePath()}}
export async function speciesStateOverlayRaster(source,u=.5,v=.5){const config=parseSpeciesMapConfig(source);if(Math.abs(config.center[0]+15)>=12||Math.abs(config.center[1]+55)>=18)return null;const point=mapCoordinate(config,u,v),data=await brazilStates(),feature=data.features?.find(item=>pointInState([point.lon,point.lat],item.geometry));if(!feature)return null;const code=String(feature.properties?.codarea||""),key=code;if(stateOverlayCache.has(key))return{...stateOverlayCache.get(key),point};const canvas=document.createElement("canvas");canvas.width=900;canvas.height=600;const context=canvas.getContext("2d",{alpha:true});context.clearRect(0,0,900,600);drawStatePath(context,feature.geometry);context.fillStyle="rgba(0,255,80,.33)";context.fill("evenodd");context.strokeStyle="rgba(190,255,205,.98)";context.lineWidth=5;context.stroke();const name=BRAZIL_STATE_NAMES[code]||code;const points=statePolygons(feature.geometry).flatMap(polygon=>polygon[0]||[]),center=points.reduce((sum,p)=>[sum[0]+p[0],sum[1]+p[1],sum[2]+1],[0,0,0]);if(center[2]){const q=brazilPoint(center[0]/center[2],center[1]/center[2]);context.font="700 25px system-ui";context.textAlign="center";context.lineWidth=5;context.strokeStyle="rgba(0,30,12,.9)";context.strokeText(name,q.x,q.y);context.fillStyle="#eaffef";context.fillText(name,q.x,q.y)}const result={canvas,name,code};stateOverlayCache.set(key,result);return{...result,point}}
async function brazilOccurrences(taxonKey){const q=new URLSearchParams({taxon_key:String(taxonKey),country:'BR',has_coordinate:'true',limit:'1000'}),r=await resilientFetch('https://api.gbif.org/v1/occurrence/search?'+q,{credentials:'omit',cache:'force-cache'});if(!r.ok)throw Error('GBIF occurrences HTTP '+r.status);return (await r.json()).results||[];}
async function brazilSpeciesMapRaster(source) {
...
async function globalSpeciesMapRaster(source) {
...
export async function speciesMapRaster(source) {
  const config = parseSpeciesMapConfig(source);
  const brazil = Math.abs(config.center[0] + 15) < 12 && Math.abs(config.center[1] + 55) < 18;
  return brazil ? brazilSpeciesMapRaster(source) : globalSpeciesMapRaster(source);
}
...
export async function speciesRegionRecords(source,u=.5,v=.5,selection=null){const config=parseSpeciesMapConfig(source),taxon=await resolveTaxonKey(config.taxon),point=selection?.point||mapCoordinate(config,u,v),radius=Math.max(.35,55/(2**config.zoom)),stateName=String(selection?.name||""),key=JSON.stringify(config)+":"+(selection?.code||point.lat.toFixed(2)+":"+point.lon.toFixed(2));if(regionCache.has(key))return regionCache.get(key);const pending=(async()=>{const base={taxon_key:String(taxon.key),has_coordinate:"true",limit:"24"};if(stateName)base.state_province=stateName;else{base.decimal_latitude=Math.max(-89.9,point.lat-radius)+","+Math.min(89.9,point.lat+radius);base.decimal_longitude=Math.max(-179.9,point.lon-radius)+","+Math.min(179.9,point.lon+radius);}const load=async type=>{const q=new URLSearchParams({...base,media_type:type}),r=await resilientFetch("https://api.gbif.org/v1/occurrence/search?"+q,{credentials:"omit",cache:"force-cache"});if(!r.ok)throw Error("GBIF records HTTP "+r.status);return r.json();},[pj,sj]=await Promise.all([load("StillImage"),load("Sound")]),photos=[],sounds=[],seen=new Set();for(const occurrence of [...(pj.results||[]),...(sj.results||[])])for(const media of occurrence.media||[]){const url=String(media.identifier||"");if(!/^https:\/\//i.test(url)||seen.has(url))continue;const item={url,reference:media.references||occurrence.references||"",creator:media.creator||media.rightsHolder||"Unknown observer",license:media.license||"",place:occurrence.stateProvince||occurrence.locality||occurrence.country||"Selected region"};if(media.type==="StillImage"&&photos.length<6){photos.push(item);seen.add(url)}else if(media.type==="Sound"&&sounds.length<4){sounds.push(item);seen.add(url)}}return{config,taxon,point,name:stateName?stateName+", Brazil":regionLabel([...(pj.results||[]),...(sj.results||[])],point),count:Math.max(Number(pj.count)||0,Number(sj.count)||0),photos,sounds}})();regionCache.set(key,pending);while(regionCache.size>16)regionCache.delete(regionCache.keys().next().value);try{return await pending}catch(e){regionCache.delete(key);throw e}}

export async function speciesRegionPanelRaster(source, u, v, selection=null) {
  const key=source+":"+(selection?.code||Math.round(u*30)+":"+Math.round(v*30));if(regionPanelCache.has(key))return regionPanelCache.get(key);const pending=(async()=>{
  const result = await speciesRegionRecords(source, u, v, selection);
