...
test("renders a transparent spectral window with an optional bounded network", async () => {
  ...
  assert.match(source, /const NETWORK_PEAKS = 28/);
  ...
  assert.match(source, /MAX_LABELS = 48/);
  assert.match(source, /MAX_LABEL_GLYPHS = 1024/);
  assert.match(source, /sampleRate \/ \(analyser\?\.fftSize \|\| 256\)/);
  assert.match(source, /modelViewMatrix \* instanceMatrix/);
  assert.match(source, /const spacing = 0\.00147/);
});
