...
test("renders a transparent spectral window with an optional bounded network", async () => {
  ...
  assert.match(source, /const NETWORK_PEAKS = 20/);
  ...
  assert.match(source, /MAX_LABELS = 18/);
  assert.match(source, /MAX_LABEL_GLYPHS = 144/);
  assert.match(source, /sampleRate \/ analyser\.fftSize/);
  assert.match(source, /billboard\.copy\(contentWorld\)\.invert\(\)\.multiply\(cameraWorld\)/);
  assert.match(source, /const spacing = 0\.0042/);
});
